...
# Cargar la alarma
pygame.mixer.music.load('alarm/alarmdos.mp3')

alarm_playing = False
freeze_polygon = False  # Variable para manejar el estado del polígono

# Cargar parámetros de calibración de la cámara
with np.load('./calib/calibration_parametersjp.npz') as data:
...
last_polygon = None
last_detection_time = time.time()  # Tiempo de la última detección de ArUcos
...
while cap.isOpened():
    ...
    if not freeze_polygon:
        ...
        if ids is not None and len(ids) >= 4:
            # Reiniciar el temporizador al detectar ArUcos
    ...
    # Si no se detectan ArUcos durante 5 segundos, borrar el polígono
    ...
    if key == ord("q"):
        ...
        freeze_polygon = not freeze_polygon  # Cambiar el estado de congelación del polígono
